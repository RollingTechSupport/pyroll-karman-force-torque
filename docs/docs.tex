...
\begin{document}
    ...
    \begin{itemize}
        ...
        \item a rigid-plastic solver based on Orowan's slab theory (\autoref{sec:orowan-model}), used for medium passes, that keeps von-Kármán's structure but replaces pure Coulomb friction with a mixed Coulomb/sticking law;
    ...
    \section{Orowan model for medium passes}\label{sec:orowan-model}

    Pure Coulomb friction, integrated all the way across the contact as in \autoref{eq:karman-elem}, demands ever-increasing shear traction toward the roll gap centre; once a pass is thick or high-friction enough, this exceeds what the material can actually transmit and produces an unrealistic, unbounded pressure spike. Orowan's slab theory~\cite{Orowan1943} keeps von-Kármán's force balance exactly as in \autoref{eq:karman-elem}, but caps the friction shear stress at the material's sticking (shear-yield) limit once Coulomb friction would exceed it, giving a friction hill that saturates ("sticking") rather than diverging. \lstinline{orowan_solver.py} implements \lstinline{OrowanSolver} using the same smoothed Bay and Wanheim~\cite{BayWanheim1976} mixed Coulomb/sticking law as the layer model (\autoref{sec:layer-model}, \autoref{eq:mixed-friction}), applied to the whole (single, homogeneous) cross-section rather than per layer.

    Like \lstinline{KarmanSolver}, this solver is rigid-plastic: it has no elastic entry/exit zones and no thermal coupling, matching Orowan's original 1943 theory - those are a separate refinement, orthogonal to the friction-law refinement Orowan introduced, and are covered instead by the layer model once a pass is thick enough to need them (\autoref{sec:dispatch}).
    ...
    \begin{itemize}
        \item the strip is discretized into $N$ through-thickness layers (used here for $N>1$, the "thick slab" regime; $N=1$ collapses to a single homogeneous slab but is not used by this plugin's dispatch, since \lstinline{OrowanSolver} covers that case without elastic zones);
        ...
        \item friction at the roll interface, and between adjacent layers, follows the same mixed Coulomb/sticking law as \autoref{sec:orowan-model} after Bay and Wanheim~\cite{BayWanheim1976} (\autoref{eq:mixed-friction}), transitioning smoothly from Coulomb sliding to a sticking-limited shear stress as normal pressure grows;
    ...
    \begin{enumerate}
        ...
        \item otherwise, the classical contact-length-to-mean-thickness ratio $L_d/H_m$ decides between the layer model's thick-slab configuration (\autoref{sec:layer-model}) and the Orowan model (\autoref{sec:orowan-model}) for medium passes.
    \end{enumerate}

    All three limits are themselves hooks (\autoref{tab:hookspecs}) and can be overridden per pass, including forcing a particular model unconditionally. \lstinline{OrowanSolver} does not itself need the elastic properties or thermal data the other refinements do, but is kept behind the same elastic-properties gate as step 1 for consistency, rather than being dispatched independently of it.
    ...
    \begin{table}
        ...
        \begin{tabular}{@{}p{0.37\linewidth}p{0.55\linewidth}@{}}
            ...
            \texttt{thick\_slab\_condition} & Whether the multi-layer, thick-slab configuration (\autoref{sec:layer-model}) is used instead of \texttt{OrowanSolver} (\autoref{sec:orowan-model}) \\
            ...
            \texttt{friction\_stiction\_coefficient} & Sticking friction coefficient $m$ feeding the mixed friction law (\autoref{eq:mixed-friction}) of \texttt{OrowanSolver} and \texttt{LayerRollingSolver} (default 0.8) \\
